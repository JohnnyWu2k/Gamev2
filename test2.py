...
def load_dynamic_function(name, default):
    """Load a dynamic function, using default if not replaced by a mod."""
    return dynamic_functions.get(name, default)

def clear_terminal():
    os.system('cls' if os.name == 'nt' else 'clear')

def display_map():
    clear_terminal_func = load_dynamic_function("clear_terminal", clear_terminal)
    clear_terminal_func()
...
def move_player(direction):
    ...
    if handle_interaction(new_y, new_x):
        update_map(y, x, " ")  # 删除玩家旧位置的标记
...
def load_map():
    ...
    if choice >= 0 and choice < len(map_files):
        ...
        with open(os.path.join(maps_dir, map_file), "r") as f:
            new_map = [line for line in f.read().splitlines()]
            maps = [new_map]
        for y, row in enumerate(maps[0]):
            if 'P' in row:
                player_pos = [y, row.index('P')]
                update_map(y, row.index('P'), " ")  # 隐藏初始位置的P
                break
        main_game_loop()
    else:
...
def main_menu():
    ...
    while True:
        ...
        if choice == "1":
            start_new_game_func = load_dynamic_function("start_new_game", start_new_game)
            start_new_game_func()
        elif choice == "2":
            load_map_func = load_dynamic_function("load_map", load_map)
            load_map_func()
...
def main_game_loop():
    display_map_func = load_dynamic_function("display_map", display_map)
    while True:
        display_map_func()
        move = input("Move (w/a/s/d): ").lower()
        if move in ["w", "a", "s", "d"]:
            move_player_func = load_dynamic_function("move_player", move_player)
            move_player_func(move)
        else:
...
def you_dead(cause=None):
    ...
    if cause == "enemy":
        ...
        print("You were defeated by a headscissors!")
    else:
        print("You are dead!")
    print("\nGame Over")
...
if __name__ == "__main__":
    # 定义所有默认函数
    dynamic_functions["clear_terminal"] = clear_terminal
    dynamic_functions["display_map"] = display_map
    dynamic_functions["play_sound"] = play_sound
    dynamic_functions["move_player"] = move_player
    dynamic_functions["handle_interaction"] = handle_interaction
    dynamic_functions["update_map"] = update_map
    dynamic_functions["level_up"] = level_up
    dynamic_functions["load_map"] = load_map
    dynamic_functions["load_mods"] = load_mods
    dynamic_functions["main_menu"] = main_menu
    dynamic_functions["start_new_game"] = start_new_game
    dynamic_functions["main_game_loop"] = main_game_loop
    dynamic_functions["you_dead"] = you_dead
    main_menu()
